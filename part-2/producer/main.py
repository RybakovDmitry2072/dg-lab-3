...
class ExamplePublisher(object):
    ...
    def add_event_task(self, city, task_type):
        self._pending_tasks.append({
            'task_type': task_type,
            'city': city
        })

    def add_weather_task(self, q, task_type):
        self._pending_tasks.append({
            'task_type': task_type,
            'q': q
        })
    ...
    def on_exchange_declareok(self, _unused_frame, userdata):
        ...
        self.setup_queue(self.WEATHER_QUEUE, self.ROUTING_KEY_FOR_WEATHER)
        self.setup_queue(self.EVENTS_QUEUE, self.ROUTING_KEY_FOR_EVENTS)
    ...
    def publish_message(self):
        ...
        routing_key = self.__get_routing_key_by_task_type(task['task_type'])
    ...
    def __get_routing_key_by_task_type(self, task_type):
        return self.ROUTING_KEY_FOR_WEATHER if task_type == 'weather'\
                else self.ROUTING_KEY_FOR_EVENTS
...
def main():
    ...
    # # Connect to localhost:5672
    example = ExamplePublisher(amqp_url)

    example.add_weather_task(q="Paris", task_type="weather")
    example.add_event_task(city="Paris", task_type="events")
    example.run()
